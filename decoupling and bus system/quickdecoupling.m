...
while max(abs(delta_V)) > epsilon && iteration < max_iterations
    ...
    for i = 1:3
        ...
        if i == 1
            ...
            J(i, 2) = V(i) * (P(i) - delta_P) * sin(angle(V(i)) - angle(V(2))) - V(i)^2 * real(Y(i, 2)) * sin(angle(V(i)) - angle(V(2)));
            J(i, 3) = V(i) * (P(i) - delta_P) * sin(angle(V(i)) - angle(V(3))) - V(i)^2 * real(Y(i, 3)) * sin(angle(V(i)) - angle(V(3)));
        end
    end
